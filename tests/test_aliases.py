...
def test_search_dict_for_alias_path():
    ...
    assert AliasPath(1).search_dict_for_path(['hello', 'world']) == 'world'


def test_validation_alias_path_with_root_sequence_index():
    class Model(BaseModel):
    ...
    assert Model.model_validate({'0': 42, 'name': 'alice', '7': 'active', '-1': 'active'}).model_dump() == {
        'id': 42,
        'name': 'alice',
        'status': 'active',
        'last': 'active',
