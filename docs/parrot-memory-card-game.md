> Preserved from the repository root README. This branch replaced that file
> with the Sing me a Song documentation; nothing here has been changed except
> the image paths, which now point one directory up.

# ðŸ¦œ Parrot Memory Card Game
...
**HTML, CSS and JavaScript** â€” no build step and no dependencies.

![Desktop preview](../img/desktop.png)

## How to play

1. On load you're asked **how many cards** to play with â€” an even number
   between **4 and 14**.
...
3. The header tracks your number of moves (**Jogadas**) and the elapsed time
   (**relÃ³gio**).
4. Match every pair to win â€” you can then start a new game.

> â„¹ï¸ The in-game text is in Brazilian Portuguese (the original author's language).

## Project structure

```text
.
â”œâ”€â”€ index.html        # markup, meta tags and asset links
â”œâ”€â”€ css/
â”‚   â””â”€â”€ style.css     # colour palette + responsive layout
â”œâ”€â”€ src/
â”‚   â””â”€â”€ script.js     # game logic (shuffle, flip, match, timer)
â””â”€â”€ img/              # parrot artwork + preview screenshots
```
...
This round of work delivered the client's two requests â€” a mobile-friendly
layout and a **white / light-green / black** colour scheme â€” plus a few small
correctness fixes.
...
| `--color-surface` | `#a7e9af` | cards / faces â€” light green                    |
| `--color-border`  | `#8ed29a` | card edges â€” same green family                 |
| `--color-text`    | `#000000` | all text (was teal `#75B79E` and gray)         |

The old teal title, lime background and gray counters are gone â€” the UI now
...
- Removed the lone `@media (max-width: 335px)` rule â€” it left typical phones
  (~360â€“430px) stuck on the desktop layout. The fluid system now covers
...
- **Card count via `prompt()`** â€” the game still asks for the number of cards
...
- **Parrot artwork is intentionally unchanged** â€” the colourful parrot images
...
- **No persistence** â€” moves and elapsed time reset on every reload; there is
  no high-score / best-time tracking.
- **UI copy is Portuguese only** â€” no internationalisation yet.
