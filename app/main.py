...
async def lifespan(_: FastAPI):
    ...
    # Build (and backfill) the FTS5 search index so /search is ready even if
    # the database already had rows from a previous import.
...
def _legacy_search(db: Session, q: str, page: int, page_size: int) -> CustomerPage:
    """Substring ILIKE search, used when FTS5 is unavailable.

    Retained as a fallback for SQLite builds compiled without FTS5 and for
    non-SQLite engines (e.g. a future PostgreSQL profile), so /search keeps
    working everywhere even though FTS5 is the primary path.
    """
    like = f"%{q}%"
    cond = or_(
...
async def search_customers(
    ...
    """Full-text search over customers, ranked by BM25 relevance (SQLite FTS5).

    Tokens are prefix-matched and combined with implicit AND. Results are
    ordered by ``bm25()`` relevance (then by id for a stable tie-break).
    ``fuzzy=true`` switches to a trigram index for substring / typo tolerance.
    Falls back to a simple ILIKE scan if the running engine has no FTS5 support.
    """
    # Non-SQLite engines (e.g. a Postgres profile) have no FTS5 -- use the fallback.
    if db.get_bind().dialect.name != "sqlite":
        return _legacy_search(db, q, page, page_size)

    caps = search_index.detect_capabilities(db.connection())
    if not caps["fts5"]:
        return _legacy_search(db, q, page, page_size)
