...
from .customer_search import (
    ...
    ensure_index,
    search_customers as fts_search,
)
...
def _legacy_search(db: Session, q: str, page: int, page_size: int) -> CustomerPage:
    """Compatibility path for databases without a usable SQLite FTS5 index."""
    like = f"%{q}%"
    cond = or_(
...
async def search_customers_endpoint(
    ...
    try:
        return fts_search(db, query=q, page=page, page_size=page_size)
