...
export default function CreateNewRecommendation({ onCreateNewRecommendation = () => 0, disabled = false }) {
  ...
  return (
    <Container onSubmit={handleCreateRecommendation}>
      <Input aria-label="Recommendation name" type="text" placeholder="Name" value={name} onChange={e => setName(e.target.value)} disabled={disabled} required maxLength={200} />
      <Input aria-label="YouTube URL" type="url" placeholder="https://youtu.be/..." value={link} onChange={e => setLink(e.target.value)} disabled={disabled} required maxLength={2048} />
