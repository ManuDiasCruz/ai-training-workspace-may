...
function parsePositiveInteger(value: string) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw wrongSchemaError("Route parameter must be a positive integer");
  }
...
async function getTop(req: Request, res: Response) {
  ...
  const limit = parsePositiveInteger(amount);
  if (limit > 100) {
    throw wrongSchemaError("Top recommendations limit cannot exceed 100");
  }

  const recommendations = await recommendationService.getTop(limit);
  res.send(recommendations);
}
