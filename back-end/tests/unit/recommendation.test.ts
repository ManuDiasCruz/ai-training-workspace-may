...
describe("UNIT TESTS SUITE", () => {
  ...
  describe("Upvotes tests", () => {
    it("Sucess in upvote", async () => {
      jest.spyOn(recommendationRepository, "find").mockResolvedValueOnce(recommendation2);
      jest
  ...
  describe("/GET tests", () => {
    ...
    it("Sucess", async () => {
      jest.spyOn(recommendationRepository, "findAll").mockResolvedValueOnce([recommendation2]);
  ...
  describe("Random recommendations tests", () => {
    it("Success in get random gt", async () => {
      ...
      expect(result).toEqual(recommendation2);
    });
    ...
    it("Falls back to all recommendations when the score filter matches nothing", async () => {
      jest.spyOn(Math, "random").mockReturnValueOnce(0.5);
      const findAll = jest
