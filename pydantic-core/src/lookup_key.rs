...
pub(crate) struct LookupPath {
    /// The first item in the lookup path.
    first_item: PathItem,
...
impl LookupPath {
    ...
    pub fn py_get_dict_item<'py>(&self, dict: &Bound<'py, PyDict>) -> PyResult<Option<Bound<'py, PyAny>>> {
        self.get_impl(
            dict,
            |d, loc| match loc {
                PathItem::S(path_item_string) => d.get_item(path_item_string),
                _ => Ok(loc.py_get_item(d.as_any())),
            },
    ...
    pub fn py_get_item<'py>(&self, obj: &Bound<'py, PyAny>) -> PyResult<Option<Bound<'py, PyAny>>> {
        self.get_impl(obj, |d, loc| Ok(loc.py_get_item(d)), |d, loc| Ok(loc.py_get_item(&d)))
    }
    ...
    pub fn simple_py_get_attr<'py>(&self, obj: &Bound<'py, PyAny>) -> PyResult<Option<Bound<'py, PyAny>>> {
        self.get_impl(
            obj,
            |d, loc| match loc {
                PathItem::S(path_item_string) => path_item_string.py_get_attrs(d),
                _ => Ok(loc.py_get_item(d)),
            },
    ...
    pub fn json_get<'a, 'data>(&self, dict: &'a JsonObject<'data>) -> ValResult<Option<&'a JsonValue<'data>>> {
        ...
        if let Some(v) = self.first_item.json_obj_get(dict) {
            ...
            if let Some(v) = self.rest.iter().try_fold(v, |d, loc| loc.json_get(d)) {
                return Ok(Some(v));
            }
        }
    ...
    pub fn json_get_value<'a, 'data>(
        &self,
        json_value: &'a JsonValue<'data>,
    ) -> ValResult<Option<&'a JsonValue<'data>>> {
        if let Some(v) = self.first_item.json_get(json_value) {
            if let Some(v) = self.rest.iter().try_fold(v, |d, loc| loc.json_get(d)) {
                return Ok(Some(v));
            }
        }
    ...
    /// get the `str` from the first item in the path, if it is a string.
    pub fn first_key(&self) -> Option<&str> {
        match &self.first_item {
            PathItem::S(path_item_string) => Some(path_item_string),
            _ => None,
        }
    }
